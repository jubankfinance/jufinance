...
interface IsJUB {
    function rebase( uint256 jubProfit_, uint epoch_) external returns (uint256);
...
contract Staking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( IsJUB( sJUB ).gonsForBalance( _amount ) ),
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            IWarmup( warmupContract ).retrieve( _recipient, IsJUB( sJUB ).balanceForGons( info.gons ) );
        }
    }
    ...
    function forfeit() external {
        ...
        IWarmup( warmupContract ).retrieve( address(this), IsJUB( sJUB ).balanceForGons( info.gons ) );
    ...
    function rebase() public {
        if( epoch.endBlock <= block.number ) {
            ...
            IsJUB( sJUB ).rebase( epoch.distribute, epoch.number );
            ...
            uint staked = IsJUB( sJUB ).circulatingSupply();
